...
class Mazecontainer extends Component {
 ...
 componentWillMount()
 {
  ...
  handlerSubmit(e) {
    ...
    if(!this.state.height) {alert("Height should be declared"); return;}
  ...
  Astar(visit,parent)
  {
    ...
    this.AstarNextStep(visit,parent);
  }
  AstarNextStep(visit,parent)
  {
    ...
    if(heap.size())
    {
      ...
      if (
        ...
        var cost=Math.abs(i+1-this.state.xEnd)+Math.abs(j-this.state.yEnd);
      ...
      if ( j + 1 < this.state.mazeWidth && this.state.Graph[i][j + 1] == "." && !visit[i][j + 1] ) {
        ...
        var cost=Math.abs(i-this.state.xEnd)+Math.abs(j+1-this.state.yEnd);
